...
mLog( "ScribbleArea" )
{
    ...
    onionRed = true;
    toggledOnionColor();
    //mView=new QTransform();
    //mTempView =new QTransform();
}
...
//会获取当前对应的层和tool，并引起tool的mousePressEvent事件
...
void ScribbleArea::mouseMoveEvent( QMouseEvent *event )
{
   ...
   // lastPixel = mStrokeManager->getLastPixel();//++我，获得最后一个Pixel
    currentPoint = mEditor->view()->mapScreenToCanvas( currentPixel );
...
void ScribbleArea::floodFill(VectorImage* vectorImage, QPoint point, QRgb targetColour, QRgb replacementColour, int tolerance) {
    bool invertible;
    QPointF initialPoint = getView().inverted(&invertible).map(QPointF(point));
    // Step 1: peforms a standard (pixel-based) flood fill, and finds the vertices on the contour of the filled area
    qreal tol = 8.0/qAbs(getView().m11()); // tolerance for finding vertices along the contour of the flood-filled area
    qreal tol2 = 1.5/qAbs(getView().m11()); // tolerance for connecting contour vertices from different curves // should be small as close points of different curves are supposed to coincide
    ...
    vectorImage->outputImage(targetImage,mEditor->view()->getView(),mIsSimplified,mShowThinLines, isEffectOn( EFFECT_ANTIALIAS )); // the target image is the vector image with simplified curves (all width=1)
    ...
    QPointF mBoxTopRight = getView().inverted(&invertible).map(    QPointF(qMax(boxLeft,boxRight)+1*tol, qMax(boxTop,boxBottom)+1*tol)  );
    QPointF mBoxBottomLeft = getView().inverted(&invertible).map( QPointF(qMin(boxLeft,boxRight)-1*tol, qMin(boxTop,boxBottom)-1*tol) );
    ...
    for(int i=0; i< contourPixels.size(); i++ ) {
        QPointF mPoint = getView().inverted(&invertible).map(  QPointF( contourPixels.at(i) )   );
            ...
            if( index2 != -1 ) {
                ...
                if(pointsNearby.size() > 0) {
                    ...
                    if(  ((vertex.curveNumber == vertex0.curveNumber) && (qAbs(vertex.vertexNumber-vertex0.vertexNumber)==1))  ||  (dist < tol2) ) {
                        ...
                        BezierArea newArea = BezierArea( closedPath, 2 );//brush.colourNumber
    ...
    replaceImage->fill(mEditor->color()->frontColor().rgba());//qRgba(0,0,0,0)
    deselectAll();
...
QPointF ScribbleArea::getOffset()
{
    return offset;
}
